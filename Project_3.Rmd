...
```{r tablePopCount, echo=TRUE, eval=TRUE,warning=FALSE,message=FALSE}
  ...
  tech.news$popularity <- ifelse(tech.news$shares < mean(tech.news$shares),
  "Unpopular", "Popular"
)

table(tech.news$popularity)
...
```{r conTable, echo=TRUE, eval=TRUE,warning=FALSE,message=FALSE}
   ...
   table(tech.news$popularity,tech.news$is_weekend)
...
```{r barPlot, echo=TRUE, eval=TRUE,warning=FALSE,message=FALSE}
  ...
  ggplot(tech.news, aes(fill= popularity, y=shares, x=num_imgs)) +
...
```{r corrPlot, echo=TRUE, eval=TRUE,warning=FALSE,message=FALSE}
    ...
    corr <- cor(select_if(tech.news, is.numeric))
    ggcorrplot(corr, 
                method = "circle",
                type = "lower",
                outline.color = "black",
...
```{r scattPlot, echo=TRUE, eval=TRUE,warning=FALSE,message=FALSE}
  ...
  ggplot(tech.news, aes(y=shares, x=n_tokens_content)) + 
