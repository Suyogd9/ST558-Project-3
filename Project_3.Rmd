...
```
```{r regressModelPredict, echo=TRUE, eval=TRUE,warning=FALSE,message=FALSE}
# convert categorical variables into numeric variables i.e one hot encoding
...
testDF <- as_tibble(predict(dummies, newdata = test)) %>%
  ...
  predResults <- postResample(predLR,obs = testDF$shares)
...
testDF <- as_tibble(predict(dummies, newdata = test)) %>%
  ...
  predResults <- postResample(predRF,obs = testDF$shares)
